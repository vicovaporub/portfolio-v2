...
export default function ProjectPage( { project } : { project: Project }) {
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8 bg-[var(--background)] text-[var(--text-primary)] theme-transition animate-fade-in-up">
            {hasImage && (
                <div className="w-full h-48 md:h-[350px] relative rounded-xl overflow-hidden border border-[var(--border)] shadow-sm mb-8 group">
                    <Image
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10 pb-6 border-b border-[var(--border)]">
                <div className="space-y-3">
                    {project.company && (
                        <span className="text-xs font-bold tracking-widest text-[var(--accent-blue)] uppercase block">
                            {project.company}
                        </span>
                    )}
                ...
                <div className="flex items-center gap-3 pt-2 flex-shrink-0">
                    {hasRepo && (
                        <a
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-[var(--border)] bg-[var(--background-secondary)] hover:border-[var(--accent-blue)] hover:text-[var(--accent-blue)] text-sm font-medium transition-all"
                        >
                    ...
                    {hasLive && (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[var(--accent-blue)] text-white hover:opacity-90 text-sm font-medium transition-all shadow-sm"
                        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
                ...
                <div className="lg:col-span-1 border-t lg:border-t-0 lg:border-l border-[var(--border)] pt-8 lg:pt-0 lg:pl-8">
                            ...
                            <ul className="space-y-3">
                                {technologies.map((technology) => (
                                    <li key={technology.id}>
                                        <Badge className="w-full flex justify-center py-2 text-sm bg-[var(--background-secondary)] border-[var(--border)] hover:border-[var(--accent-blue)] transition-colors">
                                            {technology.name}
                                        </Badge>
                                    </li>
                                ))}
