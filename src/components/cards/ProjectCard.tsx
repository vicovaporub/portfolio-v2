...
const ProjectCard = ({ project, onOpenProject }: { project: Project; onOpenProject?: (project: Project) => void }) => {
    ...
    return (
        <div
            className="group border border-[var(--tab-border)] rounded-xl shadow-lg p-3 w-full bg-[var(--tab-bg)] hover:bg-[var(--tab-hover-bg)] transition-colors duration-200 theme-transition flex flex-col md:flex-row items-start gap-3 md:gap-4 md:h-45 overflow-hidden cursor-pointer"
        ...
        >
            ...
            <div className="flex-1 flex flex-col h-full min-w-0 justify-between">
                <div>
                    <h3 className="text-lg md:text-xl font-semibold mb-1 truncate text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition-colors">
                        {project.title}
                    </h3>
                    <p className="text-[13px] md:text-base m-0 line-clamp-2 text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition-colors">
                        {project.description}
                    </p>
                </div>
