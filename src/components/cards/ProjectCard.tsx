...
const ProjectCard = ({ project, onOpenProject }: { project: Project; onOpenProject?: (project: Project) => void }) => {
    ...
    return (
        <div
            className="group relative border border-[var(--tab-border)] rounded-xl shadow-sm p-3 w-full bg-[var(--card-bg)] hover:bg-[var(--card-hover-bg)] hover:border-[var(--accent-blue)] transition-all duration-300 theme-transition flex flex-col md:flex-row items-stretch gap-4 overflow-hidden cursor-pointer h-full"
            onClick={() => onOpenProject?.(project)}
        >
            {/* Image Section */}
            <div className="w-full md:w-48 h-40 md:h-auto overflow-hidden rounded-lg bg-[var(--border)] flex-shrink-0 relative">
                <Image
                ...
                <div className="flex-1 min-h-0 flex flex-col">
                    <div className="flex justify-between items-start mb-1 flex-shrink-0">
                        <div className="flex flex-col min-w-0">
                            {project.company && (
                                <span className="text-[10px] uppercase tracking-widest text-[var(--accent-blue)] font-bold mb-0.5">
                                    {project.company}
                                </span>
                            )}
                            <h3 className="text-lg font-bold truncate text-[var(--text-primary)] transition-colors pr-2">
                                {title}
                            </h3>
                        </div>
                    ...
                    <p className="text-[13px] leading-relaxed line-clamp-3 text-[var(--text-secondary)] mb-3">
                        {description}
                    </p>
                </div>

                {/* Footer with Stack */}
                <div className="flex items-center gap-1.5 mt-auto flex-wrap flex-shrink-0">
